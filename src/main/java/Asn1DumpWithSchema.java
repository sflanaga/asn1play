...
public class Asn1DumpWithSchema {
    ...
    static void _dumpAsString(
        ...
        if (obj instanceof ASN1Null) {
            ...
            if ( node== null )
                ...
                buf.append(" [seq: fieldname: ").append(node.fieldName).append(node.primAsnBuiltinType).append("]");
            buf.append(nl);
            ...
            for (int i = 0, count = sequence.size(); i < count; ++i) {
                _dumpAsString(node, elementsIndent, verbose, sequence.getObjectAt(i).toASN1Primitive(), buf);
            }
            ...
            buf.append(" [set: fieldname: ").append(node.fieldName).append("]");
            buf.append(nl);
            ...
            for (int i = 0, count = set.size(); i < count; ++i) {
                _dumpAsString(node, elementsIndent, verbose, set.getObjectAt(i).toASN1Primitive(), buf);
            }
        } else if (obj instanceof ASN1ApplicationSpecific) {
            _dumpAsString(node, indent, verbose, ((ASN1ApplicationSpecific) obj).getTaggedObject(), buf);
            ...
            SchemaNode subnode = null;
            if ( node == null )
                buf.append(" [lost track] ");
            else {
                subnode = node.children.get(o.getTagNo());
                if (subnode == null)
                    buf.append(" [tag not in schema] ");
                else if (subnode.bottom)
                    buf.append(" [tagged fieldname: ").append(subnode.fieldName).append("] ");
            ...
            _dumpAsString(subnode, baseIndent, verbose, o.getBaseObject().toASN1Primitive(), buf);
            ...
            if (ext.getDataValueDescriptor() != null) {
                _dumpAsString(node, tab, verbose, ext.getDataValueDescriptor(), buf);
            }
            buf.append(tab + "Encoding: " + ext.getEncoding() + nl);
            _dumpAsString(node, tab, verbose, ext.getExternalContent(), buf);
        } else {
     ...
     */
    public static String dumpAsString(SchemaNode node, StringBuilder buf,
                                      Object obj) {
        return dumpAsString(node, buf, obj, false);
    }
    ...
    public static String dumpAsString(SchemaNode node, StringBuilder buf,
        ...
        _dumpAsString(node, "", verbose, primitive, buf);
        return buf.toString();
    }
